...
class TestHOTP(unittest.TestCase):
    ...
    def test_create_HOTP(self):
        timestep = 30
        message_time= hotp.create_messsage_time(30)
        logger.info('messega_time %s', message_time)
        t_key = bytes("secret key", 'ascii')
        t_text = bytes(str(message_time), 'ascii')
        t_digest = hotp.create_hmacdigt(t_key, t_text)
        self.assertTrue(hotp.HOTP_Computation(t_digest))
    ...
    def test_fixedtime_HOTP(self):
        ...
        f_bindig = hotp.create_hmacdigt(f_key, f_text)
        self.assertEqual('811807', hotp.HOTP_Computation(f_bindig))
